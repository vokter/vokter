...
public class SetSimilarityTest {
    ...
    public void jaccardTest() {
        ...
        assertEquals(1.0, s.setSimilarity(c1, c2), delta);
        assertEquals(0.7272727272727273, s.setSimilarity(c1, c3), delta);
        assertEquals(0.6363636363636364, s.setSimilarity(c1, c4), delta);
    }
    ...
    public void minHashTest() {
        ...
        assertEquals(1.0, s.setSimilarity(c1, c2), delta);
        assertEquals(0.495, s.setSimilarity(c1, c3), delta);
        assertEquals(0.94, s.setSimilarity(c1, c4), delta);
    }
    ...
    public void lshTest() {
        ...
        try {
            assertEquals(0.7272727272727273, index, delta);
        } catch (AssertionError error) {
            // maybe this was not considered to be a candidate pair, so test if
            // it's 0
            assertEquals(0, index, delta);
        }

        index = s.setSimilarity(c1, c4);
        try {
            assertEquals(0.6363636363636364, index, delta);
        } catch (AssertionError error) {
            // maybe this was not considered to be a candidate pair, so test if
            // it's 0
            assertEquals(0, index, delta);
